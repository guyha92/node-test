...
const APP_KEY = 'RGbTqpArceIXpPO9risRsYtJ1ak3hQK5Sn695SZL'
...
request.post({url:AUTH_URL, json: true, body: {
    ...
    "grant_type": "client_credentials"}}, function (error, response, body) {
        ...
        subscriptionsUrl = `${SUBSCRIPTIONS_URL}/#/customers?appkey=${APP_KEY}&utoken=${response.body.access_token}`
        console.log('subscriptionsUrl result: ' + subscriptionsUrl)
    });

console.log('waiting 1 second');
$browser.sleep(1000).then(function() {
  console.log('navigating to subscriptionsUrl customers page: ' + subscriptionsUrl)
  return $browser.get(subscriptionsUrl)
    .then(function(){
      console.log("waiting 5 seconds");
      return $browser.sleep(5000);
    }).then(function(){
      console.log("trying to find customers table");
      return $browser.waitForAndFindElement($driver.By.css(".ui-table"), 20000)
    }).then(function(el) {
      console.log("looking for customer");
      return $browser.findElement($driver.By.xpath("//*[contains(text(),'Valentin Koparov')]"));
    }).then(function(el) {
      console.log("redirecting to customer page");
      return el.click();
    }).then(function() {
      console.log("looking for customer email");
      return $browser.findElement($driver.By.xpath("//*[contains(text(),'vkoparov@yotpo.com')]"));
    })
  });
